...
-- Create database functions for dashboard data aggregation

-- Function for Partner dashboard stats
CREATE OR REPLACE FUNCTION public.get_partner_dashboard_stats(p_firm_id UUID)
RETURNS JSON
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT COALESCE(SUM(monthly_fee), 0) INTO monthly_revenue
  FROM public.clients
  WHERE firm_id = p_firm_id;

  -- Get active deliverables count (not completed)
  SELECT COUNT(*) INTO active_deliverables
  ...
  SELECT JSON_AGG(
    JSON_BUILD_OBJECT(
      'client', name,
      'revenue', COALESCE(monthly_fee, 0)
    )
  ) INTO top_clients
  FROM (
  ...
  result := JSON_BUILD_OBJECT(
    ...
    'activeDeliverables', active_deliverables,
    'teamMembers', team_members,
    'topClientsByRevenue', COALESCE(top_clients, '[]'::JSON)
  );
